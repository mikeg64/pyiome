...
def job():
                   ...
                   #https://docs.python.org/3.7/library/os.html
                    ...
                    subprocess.run(["cp", "-p iogenericsim.sh", workingdir+"/iogenericsim.sh"], capture_output=True)
					#system(command);
                    #sprintf(command,"cp -p simfile.xml %s/simfile.xml",jobdir.c_str());
                    #command="cp -p simfile.xml "+workindir+"/simfile.xml"
                    subprocess.run(["cp", "-p simfile.xml", workingdir+"/simfile.xml"], capture_output=True)
                    ...
                    subprocess.run(["chmod", "a+x", "iogenericsim.sh"], capture_output=True)
    
                    #system("./iogenericsim.sh");
                    subprocess.run(["./iogenericsim.sh"], capture_output=True)
